...
- **Add a table:** define it in `db/schema.ts` and append a migration to the `migrations` array in `db/migrate.ts` (forward-only, applied on boot; currently through `0024`). Greenfield — no back-compat needed.
...
- **Authorize:** the permission resolver is `provider/permissions.ts` — `can(action, role, group)` / `canActor(db, action, groupId, actor)` / `getMembership` / `grantsOf` / `roleHoldsAll`. **Exact membership (§5.2):** an action is allowed iff the actor's role is listed verbatim in `group.permissions[action]` — *no rank inheritance*; `owner` is the implicit super-role (always allowed). Powers need not be nested. A group declares custom roles via its `roles` catalogue (`RoleDefinition[]`); a role's grant set is derived from the permission map, not the catalogue. Kick/role-change obey the **subset (self-protect) rule** `roleHoldsAll` — a caller may act on a target only if it holds every permission the target holds (owner protected). Channel visibility: `provider/channels.ts` `channelVisibleTo(db, groupId, tier, actor)`.
