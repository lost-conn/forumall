...
pub fn HomeSidebarLayout() -> Element {
    ...
    use_effect(move || {
        if let Some(Ok(groups_data)) = groups.read().as_ref() {
            // Extract unique hosts from groups (empty string = local provider)
            let hosts: Vec<String> = groups_data
                .iter()
                .map(|g| g.host.clone().unwrap_or_default())
                .chain(std::iter::once(String::new())) // Always include local provider
                .collect::<HashSet<_>>()
                .into_iter()
                .collect();

            WS_HOSTS.write().clone_from(&hosts);
        }
    });
