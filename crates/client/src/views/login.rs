...
use crate::Route;
use forumall_shared::{try_problem_detail, ApiError, LoginRequest, LoginResponse};
            ...
            div { class: "relative w-full max-w-md",
                Card { class: "backdrop-blur-sm bg-[#1e1f22]/80 border-[#2d2f34]",
                    CardHeader {
                        ...
                        subtitle: Some("Sign in to your account".to_string()),
                    }
                    CardBody {
                        ...
                        form {
                            ...
                            onsubmit: move |e| async move {
                                ...
                                // Perform login with atomic device key registration
                                ...
                                match client.post_json::<LoginRequest, LoginResponse>(&login_url, &req).await {
                                    ...
                                    Err(ApiError::Http { status: 401, body }) => {
                                        ...
                                        error.set(Some(format!("Unauthorized (401): {msg}")));
                                    }
                                    Err(ApiError::Http { status, body }) => {
                                        let msg = try_problem_detail(&body).unwrap_or(body);
                                        error.set(Some(format!("Login failed ({status}): {msg}")));
                                    }
